#ifndef SPEEDTESTER_H
#define SPEEDTESTER_H

#include <curl/curl.h>

#include <atomic>
#include <cfloat>
#include <deque>
#include <mutex>
#include <vector>

#include "SpeedMonitor.h"
#include "SpeedSampler.h"
#include "SpeedTestConfig.h"

struct DownloadResult {
    double downloaded_mb = 0;
    double wall_time_seconds = 0;
    double aggregate_speed_mbps = 0;

    double min_speed = 0;
    double max_speed = 0;

    double smoothed_speed = 0;
    double running_average_speed = 0;

    double standard_deviation = 0;

    double p5 = 0;
    double p50 = 0;
    double p95 = 0;
    double p99 = 0;
};

class DownloadTester {
   public:
    std::mutex console_mutex;
    std::atomic<bool> stop_requested{false};
    DownloadResult run(const SpeedTestConfig& config);

   private:
    void worker();
    /*
     Non-static member functions secretly receive: this pointer
     So their signature becomes incompatible with libcurl.

     libcurl expects EXACT signature:
     size_t (*)(void*, size_t, size_t, void*)
     No hidden "this".

     static removes object dependency making callback compatible.
    */
    static size_t download_write_callback(void* contents, size_t size, size_t nmemb, void* userp);
    static int download_progress_callback(void* clientp, curl_off_t dltotal, curl_off_t dlnow,
                                          curl_off_t ultotal, curl_off_t ulnow);

    static constexpr int THREAD_COUNT = 4;
    static constexpr int TEST_DURATION_SECONDS = 5;
    std::atomic<long long> total_downloaded_bytes{0};
    std::atomic<long long> max_download_bytes{0};
    SpeedTestConfig config;
    SpeedSampler sampler;
    SpeedMonitor monitor;
};

#endif